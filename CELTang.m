...
for i=1:Nx  
    ...
    if H(i)>H(i+1) % PART I: Getting shallower        
        ...
        if H(i+1)==0 % If vertical wall                   
            ...
            for n=1:Nm
                for m=1:Nm
                    u3(n,m)=sum(phi{i+1}(m,:).*phi{i}(n,:)/H(i)*dz,2).*(k{i+1}(m)+f*l/(ii*omega))./K{i+1}(m).*exp(ii*X(i)*k{i+1}(m));  % right outgoing wave (A)  
                    u4(n,m)=sum(phi{i+1}(m,:).*phi{i}(n,:)/H(i)*dz,2).*rR(m).*(k{i+1}(m)-f*l/(ii*omega))./K{i+1}(m).*exp(-ii*X(i)*k{i+1}(m)); % right incoming wave (B)
                end
            end
        end
        ...
        if H(i)==0 % If vertical wall 
            ...
            for n=1:Nm
                for m=1:Nm
                    u1(n,m)=sum(phi{i}(m,:).*phi{i+1}(n,:)/H(i+1)*dz,2).*rL(m).*(k{i}(m)+f*l/(ii*omega))./K{i}(m).*exp(ii*X(i)*k{i}(m)); % left incoming wave (A)
                    u2(n,m)=sum(phi{i}(m,:).*phi{i+1}(n,:)/H(i+1)*dz,2).*(k{i}(m)-f*l/(ii*omega))./K{i}(m).*exp(-ii*X(i)*k{i}(m)); % left outgoing wave (B)
                end
            end
        end
...
for i=1:Nx+1
    ...
    for n=2:Nm
        ...
        if i==1
            Aeps=1;  
            Beps=exp(-(X(i)-x(ind.x))./(c{i}(n).*k{i}(n)./K{i}(n))* ...
                     nu.*(1-f^2/omega^2).*(N20-omega^2)./c{i}(n).^2);
