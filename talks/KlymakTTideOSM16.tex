...
\defbeamertemplate*{title page}{customized}[1][]
{
  ...
  \begin{center}
   ...
   \end{center}
 	\usebeamerfont{author}Harper Simmons, Shaun Johnston, Zhongxiang Zhao, Rob Pinkel,  Matthew Alford, Jennifer MacKinnon, Jonathan Nash,  Dmitry Brazhnikov, Sam Kelly, Amy Waterhouse, Nicole Jones\par	 
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Glider}
\frame{\frametitle{Gider Survey}
\framesubtitle{Shaun Johnston \& Dan Rudnick, SIO}

  \begin{columns}
      \begin{column}{0.7\textwidth}
        \begin{itemize}
          \item \emph{Johnston et al, 2015, JPO}
          \item Nov 12-Feb 13, Sep 13- Oct 13 
          \item measured CTD and velocity
          \item Estimated tidal amplitudes for $u$, $v$, and $\eta$
          \item Used gliders as antennae to separate incoming from outgoing, by fitting two plane waves over observed amplitudes and phases
        \end{itemize}
        
      \end{column}
      \begin{column}{0.3\textwidth}
        \includegraphics<1>[width=\textwidth,trim=0 0 0 0, clip]{doc/GliderTracks.png}
...
\framesubtitle{Shaun Johnston \& Dan Rudnick, SIO}
  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      \includegraphics<1>[width=\textwidth,trim=0 0 0 0, clip]{doc/Spray55.png}\\
      \includegraphics<1>[width=\textwidth,trim=0 0 0 0, clip]{doc/Spray56.png}
        
    \end{column}
    \begin{column}{0.4\textwidth}
      \begin{itemize}
        \item Two missions, fits: $\eta$, $u$, and $v$
        \item incoming towards NW
        \item reflection ENE
        \item Consistent with gross expectation
      \end{itemize}
    \end{column}
  \end{columns}
}% end frame
...
\section{Altimeter}
\frame{\frametitle{Altimetry Observations}
\framesubtitle{Zhonxiang Zhao, APL/UW \& Matthew Alford, SIO}
  \includegraphics<1>[width=\textwidth,trim=0 0 0 0, clip]{doc/AltimetryZZ.pdf}
  \begin{itemize}
    \item Analyze altimeter tracks 
    \item Decompose locally into two plane waves
    \item Note reflection ``split''  
  \end{itemize}
...
\begin{center}
  \includegraphics[width=0.85\textwidth]{./doc/EnergyFluxesAll.pdf}
\end{center}
} %\end frame
...
\end{eqnarray*}
\begin{eqnarray*}
  F_{u1}^t & = & \overbrace{u_1^ip_1^i}^{Incoming} + \overbrace{u_1^rp_1^r}^{Reflected} + 
  \overbrace{u_1^ip_1^r + u_1^rp_1^i}^{Cross\ Terms} \\
  F_{v1}^t & = & \overbrace{v_1^ip_1^i}^{Incoming} + \overbrace{v_1^rp_1^r}^{Reflected} + 
...
\begin{center}
  \includegraphics[width=0.85\textwidth]{./doc/RealResponseMode1.pdf}
\end{center}
} %\end frame
...
\begin{center}
  \includegraphics[width=0.85\textwidth]{./doc/ShelfResponseMode1.pdf}
\end{center}
}
\begin{frame}
  \frametitle{Slope Wave}
