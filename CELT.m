function [u p h A B c k uA uB pA pB]=CELT(H,X,A0,B0,N2,omega,f,nu,x,z)
% USAGE:  [u p h A B c k]=CELT(H,X,A0,B0,N2,omega,f,nu,x,z)
% Solve the Coupling Equation for Linear Tides (CELT)
%
% The solution assumes linear, Boussinesq, nonhydrostatic, f-plane mechanics.
...
% B0   [Nm  x 1]  Internal-tide forcing from right
...
% x    [nx  x 1]  Horizontal coordinates of output
...
%
% Sam Kelly, 5 MAY 2014 (smkelly@d.umn.edu)
%
...
for i=1:Nx+1;
    if H(i)~=0
        % Get bottom index
        ind.b(i)=dsearchn(Z,-H(i));
        
        % Calculate modes: solve eigenvalue problem with depth-varying stratification
        [phi{i} c{i}]=MODES(dz,N2(ind.b(i):end,i),omega);
        
        % Keep necessary modes
        phi{i}=phi{i}(:,1:Nm)';
        c{i}=c{i}(1:Nm);
        
        % Pad with zeros below bottom
        phi{i}=[zeros([Nm ind.b(i)-1]) phi{i}];
...
for i=1:Nx  
    ...
    if i==1; rL=ones([Nm 1]); else
        dx=X(i)-X(i-1);
        rL=exp(-(dx./c{i}).*nu.*(1-f^2/omega^2).*(N20-omega^2)./c{i}.^2);
    end
    
    % Viscous attenuation of incoming wave from right
    if i==Nx; rR=ones([Nm 1]); else
        dx=X(i+1)-X(i);
        rR=exp(-(dx./c{i+1}).*nu.*(1-f^2/omega^2).*(N20-omega^2)./c{i+1}.^2);
    end    
    
    % Velocity and Pressure coefficients
    if H(i)>H(i+1) % PART I: Getting shallower        
        % left velocity constraints
        u1=diag(rL.*exp(ii*X(i)*k{i})); % left incoming wave (A)
        u2=diag(exp(-ii*X(i)*k{i})); % left outgoing wave (B)      
        ...
        if H(i+1)==0 % If vertical wall                   
            ...
            for n=1:Nm
                for m=1:Nm
                    u3(n,m)=sum(phi{i+1}(m,:).*phi{i}(n,:)/H(i)*dz,2).*exp(ii*X(i)*k{i+1}(m));  % right outgoing wave (A)  
                    u4(n,m)=sum(phi{i+1}(m,:).*phi{i}(n,:)/H(i)*dz,2).*rR(m).*exp(-ii*X(i)*k{i+1}(m)); % right incoming wave (B)
                end
            end
        end
        ...
        if H(i+1)==0 % If vertical wall (treat as if flat)
            ...
            for n=1:Nm
                for m=1:Nm
                    p1(n,m)=sum(c{i}(m)*phi{i}(m,:).*phi{i+1}(n,:)/H(i+1)*dz,2).*rL(m).*exp(ii*X(i)*k{i}(m)); % left incoming wave (A)
                    p2(n,m)=sum(c{i}(m)*phi{i}(m,:).*phi{i+1}(n,:)/H(i+1)*dz,2).*exp(-ii*X(i)*k{i}(m)); % left outgoing wave (B)
                end
            end
        end
        ...
        if H(i)==0 % If vertical wall 
            ...
            for n=1:Nm
                for m=1:Nm
                    u1(n,m)=sum(phi{i}(m,:).*phi{i+1}(n,:)/H(i+1)*dz,2).*rL(m).*exp(ii*X(i)*k{i}(m)); % left incoming wave (A)
                    u2(n,m)=sum(phi{i}(m,:).*phi{i+1}(n,:)/H(i+1)*dz,2).*exp(-ii*X(i)*k{i}(m)); % left outgoing wave (B)
                end
            end
        end
        
        % right velocity constraints
        u3=diag(exp(ii*X(i)*k{i+1})); % right outgoing wave (A)
        u4=diag(rR.*exp(-ii*X(i)*k{i+1})); % right incoming wave (B)
        ...
        if H(i)==0 % If vertical wall (treat as if flat)
            ...
            for n=1:Nm
                for m=1:Nm
                    p3(n,m)=sum(c{i+1}(m)*phi{i+1}(m,:).*phi{i}(n,:)/H(i)*dz,2).*exp(ii*X(i)*k{i+1}(m)); % right outgoing wave (B)
                    p4(n,m)=sum(c{i+1}(m)*phi{i+1}(m,:).*phi{i}(n,:)/H(i)*dz,2).*rR(m).*exp(-ii*X(i)*k{i+1}(m)); % right incoming wave (B)
                end
            end
        end
    end
...
for i=1:Nx+1
    ...
    for n=2:Nm
        ...
        if i==1
            Aeps=1;            
            Beps=exp(-(X(i)-x(ind.x))./c{i}(n)*nu.*(1-f^2/omega^2).*(N20-omega^2)./c{i}(n).^2);
        elseif i==Nx+1
            Aeps=exp(-(x(ind.x)-X(i-1))./c{i}(n)*nu.*(1-f^2/omega^2).*(N20-omega^2)./c{i}(n).^2);
            Beps=1;
        else            
            Aeps=exp(-(x(ind.x)-X(i-1))./c{i}(n)*nu.*(1-f^2/omega^2).*(N20-omega^2)./c{i}(n).^2);
            Beps=exp(-(X(i)-x(ind.x))./c{i}(n)*nu.*(1-f^2/omega^2).*(N20-omega^2)./c{i}(n).^2);
        ...
        % Assemble fields
        if 1
            uA(ind.x,:)=uA(ind.x,:)+A(n,i)*Aeps.*exp( ii*x(ind.x)*k{i}(n))*phi0{i}(n,:);
            uB(ind.x,:)=uB(ind.x,:)+B(n,i)*Beps.*exp(-ii*x(ind.x)*k{i}(n))*phi0{i}(n,:);
